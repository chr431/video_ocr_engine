...
| C-05 | hybrid = decord fork 原生：TRT 走 hybrid_gpu、CPU OCR 走宿主帧。**S6 后重测**：消费端不再是共享瓶颈（av1 hybrid 引擎损耗 0%、1.37s 快过 NVDEC 33%）。**fork 4ccf889 后改判（2026-09-12）**：CPU 产能读数收敛为滑窗持续产能（唯一口径，EWMA 排空棘轮口径删除）；kick 突发治 open-GOP DPB 倾斜计划死锁（压测 24/24）；引擎 GPU 管线全片 e2e：hevc hybrid 比纯 NVDEC **快 23%**（首次显著转正）、decode-only 达并行理想 90-95%（旧口径 67-77% 是 EWMA 高估 rc 的伪影）；av1/h264 引擎内仍走显式后端更优（C-04/C-08） | 本机 4060/16C32T；**fork 开发 dll**（八提交，未发布 wheel；出厂 0.8.2 wheel 不含预路由/池深/sustained/kick，行为=旧结论） | fork 发布 wheel / 核数格局变化 | log 2026-09-11-hybrid差距分解 §8-§9；benchmarks hybrid_cpu_rate_ratchet + hybrid_sustained_default；DEPENDENCIES decord 节 |
