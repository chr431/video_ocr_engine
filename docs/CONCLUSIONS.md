...
| C-40 | **hybrid 的收益与视频长度相关**（同内容三编码族窗口扫描）：h264 **无交叉点**（500 帧起即胜，6000 帧 +111%）；hevc/av1 **交叉点在 1500~3000 帧**（≈13~25 秒 @120fps），更长则稳定领先（6000 帧 hevc +12% / av1 +40%）；短窗劣势的**暖态**量级仅 0.06~0.10s（见 C-41，进程首跑另计 ~0.2s）。对纯 CPU 臂，三编码全片均胜（1.10× / 2.76× / 1.81×） | 4060/16C32T；每臂独立子进程 + min-of-2（两臂同刻，差值消掉约 0.85s 的进程/引擎固定开销）；ocr=trt 隔离解码 | 换卡（NVDEC 与 CPU 相对速率变）/ 关键帧间隔差异大的片源 / 超短片（<500 帧） | log 2026-09-13-hybrid解码率与理论并联和 §续七 |
| C-41 | **短窗缺口的量级与归因（已更正，原"盲阶段采样块"归因被证伪）**：暖态同进程下 hybrid 相对纯 GPU 臂仅差 **+0.059s（w=1000）/ +0.096s（w=500）**，w=3000 已反超 −0.103s；进程内**首次**抽取差 +0.18~0.28s，但那属 hybrid 解码器实例化/引擎 init，与调度无关。证伪方式：把 water-filling 计划提前到首个 keyframe（速率播种，消除盲阶段 CPU 采样块）后 w=1000 0.6325 vs 0.6198、w=3000 1.5602 vs 1.4379 —— **不改善反而变差**，改动已回退 | 4060/16C32T；同进程 3 次取暖态均值（首次含 engine_init 不入账）；hevc/test6_hevc；对照臂 = hybrid 内 FORCE_SIDE=gpu | 换卡 / fork 实例化优化后重测 / 需"进程首跑"口径时单独测 | log 2026-09-13-份额旋钮修复与换DLL-A-B §四（含证伪链） |
| C-42 | 段边界的相似判定单次 **232µs**（launch 113 / memcpy+sync 109 / 排空仅 6 / Buffer 3），全片累计 ≈ 墙钟 21% —— **但它不在关键路径上**：把该判定短路成"恒不相似"（零 GPU 调用、分段结果逐位相同的窗口）后 wall **无改善**（2.849s vs 2.672s，方向为噪声）⇒ 消费者 q_get_wait 84~87% 把它完全吸收。**故以"占墙钟 21%"推算其优化收益是错的，此路不值得做** | GPU 管线；hevc/test6_hevc 6000 帧；该窗口 merges=0（短路前后段数均 2101） | 消费者不再空等的配置（OCR 极慢 / batch 极大）/ 段边界密度大幅上升的素材 / 换卡后 GPU 争用格局变化 | log 2026-09-13-merge判定代价与短路实验；ENGINE_PROFILE 分相 |
| C-43 | **fill_width 与 force_aspect 强交互**（重测修正了"关填充更准"的无条件表述，与 C-15 一致：224 保持）：生产口径 force_aspect=1.5 下 **224 是零误读档**（test5+test6 共 30664 帧零误读），降到 0 反而变差（test5 0→29、test6 0→210 误读）；**仅 force_aspect=0** 时关闭填充更准（test5 误读 28→3）。`fill_width=0` 作为**关闭填充**的加法开关保留，默认值不动 | 六片真值；真值头记生成配置（test5/6_ref force_aspect=1.5；test2 =0 且 fill_width=320） | 换片源（内容宽/字号分布变）/ OCR 模型换代 / 默认 force_aspect 变更 | log 2026-09-13-填充宽度重测；`_probe_acc_ab.py` 已自动复刻真值头配置 |
| C-44 | **计划重规划（`DECORD_HYBRID_REPLAN_PCT`，默认关）编码相关**：hevc 全片 −5.42%（符号 6/6）、h264 **+1.11%**（6/6，真回归）、av1 中性、hevc 3000 帧不可判定。机理：长片实测速率漂 CPU −3~+17%/GPU +4~+28%，冻结计划把块留在慢腿；h264 的 CPU 腿本快 3.7×，而混跑实测含自我争用 ⇒ 份额挪向 GPU 反而错 | 4060/16C32T；fork f42e22e；A/B 走 `_probe_dll_ab.py`（每臂独立进程+交错+配对差分）；PCT=25/GAP=8 | 有可靠的"哪条腿快"判据后可做按编码自动开关 / 争用结构改变 / 换卡 | log 2026-09-13-计划重规划 |
