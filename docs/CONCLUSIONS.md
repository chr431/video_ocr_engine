...
| C-42 | **段边界的相似判定是引擎生产者链上最重的一笔**：每段边界一次 grid=1 launch + sync（消费线程内串行），hevc 8340 次 × 313µs = 2.61s ≈ 墙钟 **21%**、h264 0.94s ≈ **27%**；同期消费者 q_get_wait 占 84~87%（余量极大） | GPU 管线（gray + NVDEC/CPU 解码 + TRT）；代表帧设备侧驻留 | 批量化/异步化落地后重测 / ONNX 宿主路径（无此 span） | log 2026-09-13-hybrid解码率与理论并联和 §续二；ENGINE_PROFILE 分相 |
