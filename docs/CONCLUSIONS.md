...
| C-40 | **hybrid 的收益与视频长度相关**（同内容三编码族窗口扫描）：h264 **无交叉点**（500 帧起即胜，6000 帧 +111%）；hevc/av1 **交叉点在 1500~3000 帧**（≈13~25 秒 @120fps），更长则稳定领先（6000 帧 hevc +12% / av1 +40%）。对纯 CPU 臂，三编码全片均胜（1.10× / 2.76× / 1.81×） | 4060/16C32T；每臂独立子进程 + min-of-2（两臂同刻，差值消掉约 0.85s 的进程/引擎固定开销）；ocr=trt 隔离解码 | 换卡（NVDEC 与 CPU 相对速率变）/ 关键帧间隔差异大的片源 / 超短片（<500 帧） | log 2026-09-13-hybrid解码率与理论并联和 §续七 |
| C-41 | **短窗缺口 = 慢腿首个采样块在保序交付上的暴露**（冷态 0.17s / 热态 0.37s，同一对臂两套 harness 之差源于"引擎是否预热"）：盲阶段 rc 未学得时无条件把首块交给 CPU，而 rc 需 CPU 首块落地 16 帧才就位 ⇒ 队头被慢腿串行占住。量级自洽：t_cpu(块) 0.30s − t_gpu(块) 0.13s ≈ 0.17s | 同内容 hevc 族（块≈247 帧）；fork 盲阶段"无条件 CPU 采样"策略；独立进程=冷、bench ab 热池=热 | 关键帧间隔变小（块变小则缺口缩小）/ fork 盲阶段策略变更 / 引擎预热策略变更 | log 2026-09-13-hybrid解码率与理论并联和 §续八 §续十；DECORD_HYBRID_DEBUG 路由行 |
