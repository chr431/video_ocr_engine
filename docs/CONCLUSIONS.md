...
| C-34 | S7 复评（Q4）：单视频受解码供给限制；ExtractionPool 互补配对已交付（pool.py），同温批量零增益（OCR-GPU-bound）故不推荐批量场景 | 2026-09-10 S0/S6 bench（本机 4060/8GB）；触发条件①已满足 | 出现多视频批量场景 / S5 内联后解码格局变化 / 显式立项请求 | tests/golden/bench_baseline.json；log 2026-09-10-S6性能轮 §0 |
