...
30. **文档分层管理（2026-09-08）：L0–L3 分离 + CONCLUSIONS 结论索引**
...
## 文档分层管理（2026-09-08）：L0–L3 分离 + CONCLUSIONS 结论索引

**背景**：文档行数已超过产品代码本体（md 约 5.9k 行 vs 产品代码约 5.3k 行），
约 86k tokens 中 79% 是冷叙事；同一事实多处置抄（实例：HYBRID 分档默认值
README 写 [8, 24] 而 engine_config 已改 [8, 16]，漂移存活约两周）；「勿再
投入 / 历史」类守卫语约 145 处全靠人肉维护，hybrid 迁移一轮清扫 5 个文件
127 行仍漏代码注释。旧结论无前提、无状态，过期不可见；「勿再投入」式永久
禁令会在前提变化后限制开发思路。

**决策**：按体裁分四层，层间只以指针连接——

- **L0 机器事实**：参数默认值唯一事实源 = `engine_config.py`，文档不复制值；
- **L1 结论索引**：新增 `docs/CONCLUSIONS.md`，一行一结论（状态 active /
  superseded / dead + 前提条件 + 复评触发 + 证据指针），全仓唯一允许规范性
  结论之处，体积预算测试守护；
- **L2 决策**：本文件，新条目用短 ADR 格式（背景 / 决策 / 影响）；
- **L3 历史**：PERFORMANCE.md **冻结增长**（新叙事进 `docs/log/`，一轮一个
  文件，禁止规范性语句）；ARCHIVE 维持纯冷；历史章节不回溯重写。

配套测试（`tests/test_docs_hygiene.py`）：CONCLUSIONS 体积预算、规范性字样
基线（存量豁免、增量严格；docs/log/ 直接禁止）、前瞻性文档悬空文件引用、
README 默认值与 engine_config 一致性。

**流程**：铁律 8 改写为「结论分离」（结论一行进 L1、叙事进 L3）；新增铁律 9
「迁移清扫」（依赖升级 / 大迁移落地时 grep L1 前提列，逐条翻状态）。
README 去史化：批量修订史压缩为指向 PERF §19/§21 的指针，删除钩子清单与
迁移记录缩为指针。

---

## 2026-09-11 · PI-15 门控校准：阈值必须 ≥ 本机可分辨下限（用户授权）

- **背景**：PI-15 以"三档各自中位再相减"判遥测开销（std ≤ +0.1%、full ≤ +1%）。
  本机实测该判据不可分辨被测量：同码两判给出 std −0.50% 与 +0.93%；固定
  off→std→full 顺序让末档稳定 +3.32%（实为轮内位置效应）；子进程模式 A/A 的
  |Δ|p95 达 2.33%。S6 轮已把该问题记为"§14 决策，本轮不动"。
- **决策**（用户："性能门控过严，导致误报。把门控适当放松些"）：
  1) 判据改为**同进程交替 + 档位顺序按轮轮转 + 同轮配对差分的均值**（双峰
     分布下中位数失效、均值对消位置效应）；2) 阈值按 **A/A 现测**标定，规则
     写死 `|A/A 均值偏差| + 3×SE`（上取整 0.05%，下限 0.30%）→ 本机 n=25 /
     inner=3 得 **std ≤ +0.85%、full ≤ +0.85%**；3) 判失败还需**符号多数一致**
     （≥70%）；4) §13.2 的设计目标 +0.1%/+1% 继续打印，作为可分辨时的标准。
- **影响**：门禁不再因噪声亮灯（复测：std +0.029%±0.291%、full +0.140%±0.257%，
  均通过）。严格性不降级——搬进 `tests/config/test_telemetry_cost.py`：按真实
  run 记录量重放插桩路径，断言 ≤2ms/run（分辨力高于墙钟约 10⁴ 倍），并用 AST
  测试禁掉 `psutil.Process.threads()`（本机 43ms/次）这类地雷。换机器/换窗口
...
## 2026-09-13 · 编辑护栏：934 裸 CR 清除记（自 AGENTS.md 迁出，原文存档）

✅ 2026-08-31 已清除全部 934 个裸 `\r`（提交 `fd2a76a`），PERFORMANCE.md 现为
**纯 LF 文件**，裸 CR = 0，因此**任何工具都安全**（文本模式含默认
`newline=None`、Edit 工具、二进制模式，对纯 LF 文件 100% 保真）。
...
### 处置（2026-09-13，用户拍板）：认新引擎为新基线，已按 §10.2 重录

流程：`record.py`（**全矩阵**，非 `--case`——F-8 冷池态坑）→ 28 用例全部
`degraded=None` → `record.py --verify` **28/28 一致** → manifest 更新（28 用例）。

**变更性质（逐片核实，非抽样）**：对 28 个 `case-*/stage-ocr.json` 逐例比对
HEAD 旧向量 vs 新向量：
...
| 逐段文本有差异的用例 | **0** |
| 段数不同的用例 | **0** |
| 置信度数值不同的段次 | 8661（全矩阵累计） |

即：**识别文字一字未变，只有置信度浮点数值漂移**——与"TRT builder 重建后
tactic/TF32 取值不同"的机制自洽。故本次重录**不触碰正确性门禁语义**
（铁律 2 的判据 = 段数 + 唯一文本集，两者皆保持），只是把"引擎产物指纹"
更新到当前本地引擎。

⚠️ 仍未解决的根因（下一轮）：`ocr_engines/` 被 .gitignore → **金标基线依赖
一个本地、不可重建的二进制**。今日一次 `-W error` 跑就把 b18 引擎重建、
令 22 例 `ocr.segments` 漂移。这不是本项目独有（TRT builder 非确定性），
