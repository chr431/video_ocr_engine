...
## 4. 混合解码 / 混合 OCR（已弃用，默认关闭，仅保留兼容）

> **2026-08 结论**：在 subtitle 场景中，纯 `CPU+CPU` 与 `GPU+TRT` 路径处理速度
> 已可比，混合方案不再有明确收益；且它增加双解码器/双引擎并发复杂度，收益不确定。
> 因此标记为**弃用（deprecated）**：代码和 env 钩子暂保留以兼容旧实验/测试，但
> 不再推荐使用，也不继续投入优化。
