...
| 方案 | 耗时 | 相对单 auto |
|---|---:|---:|
| 单流水线 auto+auto（GPU+TRT） | 7.34s | 1.00× |
| 双流水线默认（auto+auto ∥ cpu+cpu，4 片） | 5.12s | **0.70×（-30%）** |
| 双流水线自定义（cpu+auto ×2，4 片） | 4.91s | **0.67×（-33%）** |
| 双流水线默认，2 片 | 5.30s | 0.72× |
| 双流水线默认，8 片 | 5.77s | 0.79× |

补充观察：

- 默认双流水线（主+互补）在本场景有实际收益；显式两条 `("cpu","auto")`
  更快一点，但这依赖“CPU 软解 + TensorRT”是当前机器最优单流水线组合。
- 分片数 2~4 最优；8 片因额外 seek/边界处理变慢。`dual_pipeline_chunks`
  仍保留给需要更多动态负载均衡的场景。
