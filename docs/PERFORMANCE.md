...
1. **TRT⊕ONNX 共存推理互相膨胀（核心发现）**。新三国01 窗口（1889 段、
   宽 ROI ~48×780 输入）：TRT 单跑 2.89ms/段、ONNX(16T) 单跑 16.49ms/段；
   两引擎共存时双方都收敛到 ~8.4ms/段——总吞吐被钉死（与后端组合无关的
   共享串行资源特征）。而 TRT+TRT 共存无膨胀（合计≈单跑 5.46s）、
   ONNX+ONNX 共存各自维持单跑水平。
