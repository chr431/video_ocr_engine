...
>
> **2026-09-08 冻结**：本文停止追加新章节，新实验叙事写 `docs/log/`
> （一轮一个文件，见其 README 约定）。结论的**当前状态**改由
> `docs/CONCLUSIONS.md`（L1 索引）承载——本文各章保留实测细节与证据链；
> §5 / §6 / §17 的结论已收编入索引，以索引状态为准。下表「性质」列的
> Active 字样指 2026-09-08 时点。
>
> | 章节            | 性质                     | 怎么用                           |
> | ------------- | ---------------------- | ----------------------------- |
> | §1–§15, §17, §19–§24 | 事实与实测细节（时间顺序，追加至 2026-09-08 冻结） | 查证据链；结论状态 → CONCLUSIONS.md |
> | **§16** 路线图归档 | **2026-08-29 快照**      | → ARCHIVE.md；开头有**校正表**，7 处已被后续提交推翻，正文保持原样 |
> | **§17** 下一步候选 | 现役待办（2026-08-30 审查）    | 动手前先看这里                       |
> | **§22–§23** hybrid 项目层调度 | **已删实现的历史记录**（2026-09-06 迁移 decord 原生） | 现状见 **§24** 迁移记录；勿据此调参 |
...
> **2026-09-06 起失效**：`HYBRID_MAX_CHUNK_FRAMES` 及整个项目层 hybrid 实现
> 已随混合解码迁移 decord 原生实现而删除（见 §24）。本节保留为历史证据链。
...
> **2026-09-06 起失效**：`HYBRID_CALIB_ROUNDS` 等开关已随项目层实现删除
> （见 §24）。本节保留为历史证据链。
...
> **2026-09-06 起失效**：`HybridDecoder` 及其第二 reader 已随混合解码迁移
> decord 原生实现整体删除（见 §24）。本节保留为历史证据链。
...
> **2026-09-06 更新**：混合解码已迁移 decord 原生实现（§24）。`HYBRID_CPU_THREADS`
> 旋钮保留但自动分档改为钳 **[8, 16]**；本节 A/B 数据测于已删除的项目层实现，
> 方向性结论仍可参考。
...
4. 🔧 **现役代码已经做对了**：`hybrid` 混合解码用**速率比例分界 + 两端连续
   ...
   （2026-09-06 追注：该动态调度已随项目层实现下沉为 decord 原生混合解码，
   见 §24；"勿引入静态双流水线"的结论不变。）
5. 🧹 **归因收口**：并发退化的因果链为
...
> **⚠️ 已删实现的历史记录（2026-09-06）**：本节调参的项目层实现
> （`hybrid_decode.py` v5）已随混合解码迁移 decord 原生实现整体删除（§24）。
> 正文保留为历史证据链与设计依据，**勿据此调参**。
...
### 22.10 第五轮：再证伪四个假设（含一次"诱人的手算"）

承接 §22.9：CPU 明明闲着（2~3 核），hybrid 却比纯 CPU 慢 70%。本轮继续找。

**关键新观察** —— 两端在 hybrid 里**都被钉在 ~950fps**：

    cpuT=12: calib gpu=708 cpu=855 → fast=cpu，cpu 9 片 2691 帧 → 2.662s
    cpuT=24: calib gpu=916 cpu=840 → fast=gpu，gpu 9 片 2691 帧 → 2.969s
    cpuT=32: calib gpu=899 cpu=794 → fast=gpu，gpu 9 片 2691 帧 → 2.976s

不论哪端拿到大头（9 片 / 2691 帧），耗时都是 2.7~3.0s ≈ 2691/950。
且**标定极不稳定**：gpu 三次读数 708 / 916 / 899 fps（±30%），导致 fast
归属在 cpu 与 gpu 之间来回翻转。

**本轮证伪的四个假设：**

| 假设 | 实测判据 | 结论 |
|---|---|---|
| GIL 争抢（调小 switch interval 缓解） | 5ms→2.887s、0.5ms→3.034s、0.05ms→3.077s，**越调越慢** | ✗ |
| 分片数不合适 | MAX_CHUNKS 2/4/8/16/32 → 3.304/3.071/3.023/3.008/3.188s，默认 16 已近最优 | ✗ |
| 标定样本太小（噪声大） | calib 40/96/192 帧 → 3.028/3.064/3.201s，**加大反而略差** | ✗ |
| 分界目标函数不对（改最小化 max） | 见下，**5 次重复实测无差异** | ✗ |

**「诱人的手算」—— 本轮最值得记的一条教训。**

看单次实测样本（counts：前 8 片 2429 帧 + 后 6 片 1571 帧；rf=855 / rs=708 /
disc=1.0），手算不同慢端片数 k 的完工时间：

| k(慢端片数) | t_slow | t_fast | max |
|---|---:|---:|---:|
| 6（现行取） | 2.219 | **2.841** | **2.841** |
| 7（被 break） | 2.648 | 2.485 | **2.648** ← 快 6.8% |

结论看起来很硬：`safety=0.95` 要求慢端比快端早 5% 收工，于是循环在均衡点
**前一步** break，永远停在"快端是瓶颈"的一侧。改成最小化 max 即可越过。

**但 5 次重复实测没有差异**：min-max 的 decode 中位 2.589s vs 现行 2.603s
（−0.5%，噪声内），墙钟同样无差异。原因是**工作窃取已自愈了大部分失衡**，
而标定噪声（±30%）才是主导。

⇒ **已回退**（`_dynamic_split` 的 docstring 里留了这条记录）。
**教训：别只看单次手算就改并发热路径的目标函数** —— 单次样本会把噪声
当成规律，而热路径的改动需要重复实测背书。

**累计证伪（五轮共 10 个假设）**：NVDEC 争用 / host CPU 饱和 / decord 全局锁 /
生产者批量 / 双向窃取 / inflight 限流 / 内存带宽 / GIL 争抢 / 分片数 /
标定样本量 / 分界目标函数。

**仍未解释**：h264 上 hybrid(~3.0s) 比纯 CPU(~1.8s) 慢 70%。调度层面
找不到解法；最可能是结构性的（h264 上 NVDEC 比 CPU 流水线慢 2.5~3 倍，
按速率比混合 = 用慢端稀释快端），属于**后端选择**问题而非调度问题。

**建议的下一步（供决策）**：
1. 接受现状 —— hybrid 已比 `auto` 默认值快 15~28%，三个视频一致，文本 100% 相同。
2. 若追求 h264 的最优（4.1s→1.7s），需要动 `auto` 的后端选择，即引入某种
   动态判据。可行的轻量方案：把 hybrid 的**标定结果**暴露出来（它已经实测了
   两个后端的并发速率），在"某端速率明显更高"时直接退化为单端 —— 注意
   §22.4 的批量陷阱（40 帧测不准 CPU 真实速率）。
3. 转向其它方向（如长视频 / stride>1 场景），那里固定开销被摊薄，
   hybrid 的边际吞吐优势（+4.7%）更能体现。

## 23. hybrid 在线移界：原型「多轮动态分配」的单遍等价物（2026-09-05）

> **⚠️ 已删实现的历史记录（2026-09-06）**：在线移界（`HYBRID_MIGRATE`）属
> 项目层实现，已随混合解码迁移 decord 原生实现整体删除（§24）。正文保留为
> 历史证据链，**勿据此调参**。

### 23.1 假设：份额错配是校准偏差的直接后果，可在线修正

原型（repo/hybrid_decoder_prototype）靠多轮重分割收敛到"两端同时完成"；
主仓库单遍解码没有重来的机会，只能依赖一次性并发校准。本轮把该机制
改造成**单遍等价物**：每片完成后用两端**生产实测**速率重算未认领片的
最优分界，把过量份额从"标签快端"让给慢端（`HYBRID_MIGRATE=1`，默认开）。

**错配成本的量化**（§22 探针复测，h264 test5 4000 帧）：

    运行A: calib gpu 716 > cpu 673 → GPU 拿 67% → GPU 路 3.0s 拖尾（~950fps=NVDEC上限）
    运行B: calib cpu 1023 > gpu 712 → CPU 拿 70% → CPU 路 2.8s 拖尾（~1000fps）
    按生产真速率（cpu 1000 / gpu 970）均衡分割 → decode 应为 ~2.05s

两次错法不同、尾部同为 ~2.8s；一次性校准的代价 ≈ 0.7s（+37%）。
分界目标函数不是问题（min-max 裸移植 A/B 持平，§22.10 第三次应验），
喂进去的**速率**才是。

### 23.2 实现（hybrid_decode.py `_migrate_boundary` + `_maybe_migrate`）

- **只动未认领片**：两端各自按序认领 → 未认领片在 fast 区构成后缀
  [p_f, k)、slow 区构成 [q_s, n)。在 [p_f, k) 内找新分界 m 最小化
  max(两端剩余完成时间)；in-flight 剩余按 delivered+len(data) 实际
  进度折算。移界不破坏连续扫掠，接收端至多 1 次反向 seek；快端窃取
  保留作反方向纠正（对 fast 恒为无缝前向）。
- **防呆**：慢端已退出不移界（防片搁浅）；fast 至少保留 1 片未认领
  （防"全让光→窃取抖回"）；移界后冷却防震荡。

### 23.3 三轮 A/B（4000 帧 ×3 runs，同轮背靠背配对）

| 轮 | 配置 | h264 | AV1 | HEVC |
|---|---|---|---|---|
| 基线(§22.1/本轮) | MIGRATE=0 | 2.86~2.92s | 2.20~2.23s | 1.93~1.94s |
| R1 EMA混合先验+5%门槛 | 0 / 1 | 2.893 / 3.014 | 2.231 / 2.259 | 1.932 / 1.972 |
| R2 替换先验+冷却1+5% | 0 / 1 | 3.142 / 3.095 | 2.201 / 2.294 | 1.920 / **2.224** |
| R3 **seek样本剔除+双端≥2干净样本+15%门槛** | 0 / 1 | 2.919 / **2.762** | 2.201 / 2.200 | 1.942 / 1.944 |

R1 全线略差 1~4%（min 两视频更好）；R2 更糟（HEVC −15.8%）；
R3 h264 **+5.4%**、其余持平。跨轮机器漂移 ±8-10%（同配置 R2 3.142 vs
R3 2.762），**只有同轮背靠背配对可信**。

### 23.4 两个真实机制（R2→R3 的教训）

1. **seek 后遗症污染速率样本**：反向 seek 落 GOP 中段时，参考帧重建
   成本记在解码里而非 seek_s，该片实测速率暴跌（探针实录 rf=486fps、
   rs=534→655 剧烈摆动）；移界本身制造 seek → 坏样本 → 再移界，
   **死循环三连移界拖到 4.17s**（基线 2.9s）。修法：t_seek>5ms 的片
   不进 EMA；两端各 ≥2 个干净样本才允许移界。
2. **改善门槛必须高于噪声**：±10-30% 的片级速率噪声下，5% 门槛会让
   移界被噪声牵着走；15% 门槛后 R3 仅在真错配时触发（h264 探针实录：
   一次移界 8→7，rf=1119/rs=946 读数干净）。

### 23.5 结论

- 在线移界（默认开）在 h264 上同轮 A/B **+5.4%**（2.919→2.762s），
  AV1/HEVC 持平不回归；段数/唯一文本逐位一致（1294/264、1324/276、
  1282/527）。收益上限受限于：纠错窗口只有 ~14 片的认领节奏 + 15%
  门槛放弃小修正 + shed/窃取各 1 次 seek 的固定代价。
- **不改变 §22 的结构性结论**：h264 上 hybrid(~2.8s) 仍显著慢于纯
  CPU(~1.6s)——移界修的是"份额错配"层（§23.1），修不了"生产者速率
  被管线拉低"层。"hybrid 永不输最快单路"仍需后端选择（单端守卫）。
- 原型思路的可迁移部分至此收敛完毕：调度（分界算法）无增益（§22.10）、
  多轮自适应的单遍等价物 = +5.4%（本节）、其余差距在测量口径与结构。

---

## 24. 混合解码迁移 decord 原生实现（2026-09-06，迁移记录）

> 本节是混合解码的**唯一现役描述**。§10.4 / §10.5 / §12.3 / §17.2 / §22 / §23
> 描述的项目层机制（kfe 分片、并发校准、稳态折扣、在线移界、快端接管/窃取）
> 已随 `hybrid_decode.py` 整体删除，正文仅作历史证据链与设计依据保留；
> 决策过程见 `docs/DECISIONS.md` 同名章节。

**迁移内容**（commit `e5d04c3` → `fd3bcda`）：

- `decode_backend="hybrid"` 只把解码参数透传给 decord fork（≥v0.7.15）的原生
  设备上下文，项目层双 reader 壳不再存在；等价调度由 decord 内部单 demux 流
  的关键帧 chunk 路由 + 生产速率实测 min-max 积压贪心承担。
- OCR on GPU（TRT）走 `hybrid_gpu`：输出帧驻留显存，`gpu_pipeline` 设备指针
  通路直接可用，帧全程不落 RAM；OCR on CPU 走 `hybrid`：输出宿主帧（与
  `cpu()` 同布局）。打开失败降级纯 GPU（`meta.degraded_reason` 有记录）。
- 存活旋钮只剩 `HYBRID_CPU_THREADS`（默认 0 = 逻辑核×3/4 钳 [8, 16]，
  分档思想延续 §17.2）；`HYBRID_MAX_CHUNKS` / `HYBRID_MAX_CHUNK_FRAMES` /
  `HYBRID_PROBE*` / `HYBRID_CALIB_*` / `HYBRID_SLOW_*` / `HYBRID_MIGRATE` /
  `HYBRID_SOLO_GUARD` 等项目层参数全部删除。
- 同步删除：`hybrid_decode.py`（1075 行）、`tests/decode/`（5 个壳测试）、
  引用壳的 probe 工具（`_probe_hybrid_prodtime.py`、`_probe_io_budget.py`）。

**迁移时验证**（提交记录）：引擎 98 单测全过；三编码 e2e（hevc/h264/av1，
`nvdec` vs `hybrid`，TRT + gpu_pipeline 全程显存）帧数与文本一致，hybrid 侧
hevc **1.42×** / h264 **1.22×** 加速。

**与 §22 / §23 的关系**：§22 的结构性结论（NVDEC 单硬件单元会话串行、
编码反转、CPU 生产者线程预算）与 §23 的在线移界思想，已由 decord fork 内部
实现以不同形式承接；decord 侧调度的后续演进见 fork 仓库，本文不再追踪。
§22.4 / §23 遗留的"h264 上 hybrid 慢于纯 CPU"测于旧实现，原生实现的编码
适配情况以 decord 侧实测为准。
