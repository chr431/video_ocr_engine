...
**余量是硬要求**（`OCR_ROI_AUTOCROP_MARGIN`，占 ROI 宽 %，**默认 20**）：
太小会切到字符边缘笔画，太大会把留白又放回来、吃掉收益。
...
**数字场景的余量扫描**（生产路径，`_probe_truth_env.py --dbe auto`，
...
测试（左留白 10.6%）在余量 10 时退化 2 —— 余量恰好等于留白，边界上切到了
笔画；20 时恢复。test5/test6 在余量 30 时反而 0→10：宽 ROI 上余量太大把
留白又放了回来。**20 是唯一在所有视频上都不差于不裁的点。**
