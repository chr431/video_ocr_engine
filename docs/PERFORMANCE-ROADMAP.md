...
| 项 | 实测收益 | 为什么不默认开 | 开关 |
|---|---|---|---|
| **P0-6** 关去块滤波 | HEVC **-8%~-14%** 墙钟；5 片准确率均值 −0.01pp | 改变输出像素，`rep_crop` 预览有块状伪影 | `DECORD_SKIP_LOOP_FILTER=all` |
...
| # | 项 | 工作量 | 预期 | 备注 |
|---|---|---|---|---|
| 1 | **P0-6 翻成默认开**（待你拍板） | 1 行 | HEVC -8%~-14% | 唯一代价是 `rep_crop` 预览有块状伪影。若以批跑为主、不在意预览，直接翻 |
| 2 | **P0-4 扩展到 GPU 直通路径** | 中（改 `_gpu_kernels.process_gray_raw`） | 让 `decode=auto`（默认）用户也拿到 P0-4 | 现仅宿主路径生效；kernel 目前假设批内 `src_w` 一致，需按项传宽度数组 |
| 3 | **P1-3 解耦 GPU OCR 管线与 NVDEC** | 中（2~3 天） | 让 P0-1 的解码收益与零拷贝 OCR 叠加 | 目前两者互斥，详见 §3 |
| 4 | **P3' 定点下沉 `_cluster_win3`** | 中 | 现有 ROI 3.4%~5.6%；大 ROI（≥10 万像素）**10×** | 唯一值得下沉的函数，且只在大 ROI 划算 |
| 5 | hybrid 启动开销 | 小 | ~0.25s（第二 reader 开 0.12s + 校准 0.13s） | 生产者本身已接近理论最优 |
| 6 | P2-2 自写 C++ 解码层 | 2 周+ | 先做完 2/3 再评估 | 每帧 ~0.15ms 固定开销，小 ROI 有量级空间 |

**一句话**：解码侧还剩**去块滤波**这一块肉（HEVC -8~14%，等你拍板要不要翻默认）；
OCR 侧剩**把宽度裁切铺到默认路径**（2 号）；结构性的大票是 **P1-3 解耦**（3 号）。
...
**仅宿主路径生效**——GPU 直通的 `process_gray_raw` 假设批内 `src_w` 一致，
要支持需改 kernel（按项传宽度数组），留作后续。
...
### P1-3 解耦 GPU OCR 管线与 NVDEC —— 🔜 下一步候选 #3（未验证）
...
0. **🔜 候选 #2**：`process_gray_raw` 支持逐项不同宽度（按项传宽度数组），
   让 P0-4 的宽度裁切在 `decode=auto`（默认路径）上也生效。
   （现仅宿主路径生效 → 默认用户拿不到 P0-4。）
...
| 7 | **P0-6** 关去块滤波（fork） | ⏸ 已实现，env 门控默认关，待拍板 |
| 8 | 打通 fork 构建（Ninja 生成器） | ✅ 解除所有 fork 侧优化的阻塞 |
| — | ~~P0-3~~ auto 自动选型 | ❌ 用户决定不做 |
| — | ~~P1-1~~ 真跳帧 | ❌ 实测证伪（1.03~1.48×，远低于 2~4× 预估） |
| — | ~~P3~~ 全量 C 重写 | ❌ 收益上限 3~6% |

### 6.2 下一步（与 §0.4 同源，此处给出验收方式）

| # | 项 | 验收方式 |
|---|---|---|
| 1 | **P0-6 翻默认开**（待拍板） | `tools/_probe_truth_env.py --cases "关:\|开all:DECORD_SKIP_LOOP_FILTER=all"` 复跑 HEVC 两片；确认可接受 `rep_crop` 伪影 |
| 2 | **P0-4 扩到 GPU 直通路径** | 改 `process_gray_raw` 按项传宽度数组；用 `_probe_autocrop_ab.py` 在 `decode=auto` 下复测，文本必须与 `decode=cpu` 一致 |
| 3 | **P1-3 解耦 GPU OCR 与 NVDEC** | 见 §3；目标是 CPU 解码 + 零拷贝 OCR 同时生效 |
| 4 | **P3' 定点下沉 `_cluster_win3`** | 见 §5.2；**先确认 ROI ≥10 万像素**，否则只有 3.4%~5.6% 不值得 |
| 5 | hybrid 启动开销 | 见 §3 P1-2 剩余项 |
