...
### 8.4 🔜 #4 fork：NVDEC 逐帧同步去除（文档在案、从未尝试；成本前提已变）

P2-1/P2-2 的分析在案（**~0.15ms/帧固定开销** = cuvid Map/Unmap 逐帧同步 +
NVDEC 逐图提交延迟；1080p 占 14%、**标清占 53%**），当时以"自写解码层
成本 2 周+"否决。**前提已变**：fork 构建 15~21s（第四轮打通）、fork 属
自有资产、"自写层"与"改现有 fork 的输出路径"成本差一个量级。这是 NVDEC
主路径（现役 decode=auto 默认）**最后一块大肉**。方向：cuvid 输出
Map 批量化/延迟 Unmap、输出池直接对接引擎缓冲（消灭 D2D 聚批中间拷贝）。
工作量中（fork C++，等价性门槛 = 四重比对 + 全片文本逐位）；预期
标清 NVDEC 场景 -30~-50%、1080p -10~15%。
