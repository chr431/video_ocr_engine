...
| **P0-2** host 输入的 TRT 批也走 GPU argmax 归约 | ~6 行（复用现成 `execute_device_argmax`） | **墙钟 -17.6%、ocr.infer -24.1%**，结果逐位一致 | ✅ **已落地** |
| ~~P0-3~~ `auto` 后端按「编码 + 核数」选择 | — | **决定不做**（见下） | ❌ 已否决 |
| **P0-4** OCR 输入宽度自适应裁切（宽 ROI 字幕） | ~30 行 + 1 个常量 | OCR 侧 **-23.9%**（10% 余量下文本 100% 一致） | ✅ 已验证 |
...
**自动失效**（无收益则不动）：内容满宽、裁后宽 ≤ `OCR_PAD_WIDTH_MIN`(224)、
`force_aspect > 0`（宽度被强制，裁切只改缩放）。
→ 窄 ROI（如 test5 106×33）天然无收益，不会误伤。
...
**副作用（意外的正向）**：按帧对齐真值（`tools/_probe_autocrop_truth.py`，
test5 7223帧/2492段）测窄 ROI 且**手动关掉守卫**时：

| | 全等准确率 | 数值容错准确率 | 墙钟 |
|---|---:|---:|---:|
| 不裁 | 97.843% | 98.499% | 3.780s |
| 裁 + 分组 | **98.703%（+0.86pp）** | **99.359%（+0.86pp）** | 3.825s（+1.2%） |

裁切把输入变得更贴近模型训练分布（文字填满图像），**识别更准**——
那些文本变化（`8日→88`、`日1→81`）是纠错不是退化。
现役的"pad 下限守卫"会跳过窄 ROI，因此默认**拿不到**这 0.86pp；
它属于质量变更而非性能变更，要不要放开另议。
