...
- hybrid 已迁 **decord 原生**；fork 十一提交**已随 v0.8.3 发布**（sustained
  ...
  全片单臂分母 + 交错配对；死锁判别走引擎全片（§10.2）。**fork 4cebeef
  并联缺口收口（C-45）**：ROI 银行解耦 + 计划滑动视界，fork 级对并联和
  77/68/88%→87/78/90%，引擎 hevc −4.8%/av1 −2.9%/h264 平价；残余=每臂
  混跑干扰（CPU busy-rate −15~17%）；REPLAN_PCT 已删除
