...
`tools/` 现有 **56 个 `.py`**（11,357 行 / ~443 KB））））））））），其中 42 个是探针
...
| `_probe_hybrid_prodtime.py` | 105 | hybrid 生产者线程时间分解：get_batch（ctypes 期间，GIL 放开）/ asnumpy / 其他。实测 h264 生产者 get_batch 2.70s vs asnumpy 0.006s → Python 记账非大头，瓶颈在解码本身与份额错配 |
