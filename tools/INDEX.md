...
`tools/` 现有 **76 个 `.py`**（12,702 行），其中 62 个是探针
...
| `_probe_hol_stats.py` | 205 | 走 fork **非打印** `DECORD_HYBRID_STATS`（零打印扰动）实测队头阻塞时长/episode/搁置峰值 + BuildPlan 冻结速率/CPU 折数/时刻；decode-only 每格独立子进程+硬超时，`--set KEY=VAL` 做单变量 A/B | log 2026-09-11-hybrid差距分解 §8；knowledge `hybrid_cpu_rate_ratchet` |
| `_probe_stress_harness.py` | 102 | hybrid 死锁/性能**压测 harness 通用规格**：每 trial 独立子进程 + 硬超时 + 进度逐行落文件（慢消费者 = get_batch+asnumpy 保留；"无超时压测烧千秒"三踩后的纪律固化，rules.yaml 同名规则）。kick 突发修复 24/24 证据 | log 2026-09-11-hybrid差距分解 §8.2/§9.1；knowledge `hybrid_sustained_default` |
