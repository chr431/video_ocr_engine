...
`tools/` 现有 **102 个 `.py`**（17,261 行），其中 92 个是探针
...
| `_probe_ffmpeg_min_perf.py` | 132 | 2026-09-15 | **FFmpeg 极简集 vs GPL 全家桶解码性能对照**（健壮性验证）：双 decord 包副本 + 子进程 worker 全片 ROI gray，三码×双臂交错 min-of-3；NVDEC 健全性对照。结论=CPU 软解 −0.4~−1.6%（av1 含 dav1d vs native 换算）、NVDEC 零差异——瘦身无实质回退（log 见 bench/ffmpeg_min_perf.log） |
