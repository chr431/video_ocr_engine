...
`tools/` 现有 **99 个 `.py`**（16,771 行），其中 89 个是探针
...
| `_probe_acc_ab.py` | 272 | 2026-09-12 | **准确率 A/B harness**：参数化 env 旋钮跑真实引擎全片比对（`--knob OCR_GAMMA=2.0,1.0`），落 `bench/acc_ab_<knob>.json` |
...
## 探针状态（L2）：live 13 / frozen 75（共 88）
