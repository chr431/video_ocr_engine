"""text_extract_cli — 通用引擎的简易文本提取 CLI（演示/测试程序）。

在 video_ocr_engine.FieldExtractor 之上包装一个最简 CLI：
...
CLI 只暴露基本参数（视频输入、ROI、开始帧、结束帧、输出文件）；解码/OCR 后端
用演示默认（decode=auto 自动 GPU/CPU、OCR=cpu 走 ONNX），不暴露引擎细节。

用法:
    python text_extract_cli.py <video> --roi X1 Y1 X2 Y2 \
        [--start-frame N] [--end-frame N] [-o out.csv]
"""
...
from video_ocr_engine import ExtractionResult, FieldExtractor

PROG = "text_extract_cli"
...
def default_output_path(video: Path) -> Path:
    """默认输出：<视频目录>/<视频名>_ocr.csv。"""
    return video.with_name(video.stem + "_ocr.csv")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog=PROG,
        description="基于 video_ocr_engine 的简易文本提取：视频 ROI → 时间戳+OCR 文本 CSV。")
    p.add_argument("video", help="视频输入文件")
    p.add_argument("--roi", nargs=4, type=int, required=True,
                   metavar=("X1", "Y1", "X2", "Y2"),
                   help="识别区域 (x1 y1 x2 y2)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 演示默认：decode=auto（GPU 优先，回退 CPU）、OCR=cpu（ONNX，无需 TRT 构建）
    ex = FieldExtractor(
