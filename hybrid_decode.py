...
def _dynamic_split(counts: list[int], rf: float, rs: float, *,
    ...
    约束前提下给慢端尽量多的片（慢端贡献最大化）。慢端稳态速率 = rs × 折扣。
    返回 split_idx ∈ [1, n-1]（两端各至少 1 片）。

    ── 已尝试并**证伪**的替代目标：最小化 max(t_fast, t_slow) ──────────
    2026-09-01 曾改成"最小化两端完工时间的较大值"，理由是 safety=0.95 会在
    均衡点前一步 break，停在"快端是瓶颈"的一侧。手算某次实测样本
    （counts 前 8 片 2429 帧 + 后 6 片 1571 帧，rf=855 / rs=708，disc=1.0）：
        k(慢端片数)   t_slow   t_fast    max
        6（当前取）    2.219    2.841   2.841
        7（被 break）  2.648    2.485   2.648   ← 快 6.8%
    看着很诱人，**但 5 次重复实测无差异**：min-max 的 decode 中位 2.589s
    vs 当前 2.603s（−0.5%，噪声内），墙钟同样无差异。原因：工作窃取已自愈
    大部分失衡，而标定噪声（gpu 读数 708~916 fps，±30%）才是主导。
    ⇒ 已回退。**别只看单次手算就改并发热路径的目标函数。**
    """
    n = len(counts)
    if n <= 1:
        return n
