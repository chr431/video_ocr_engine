...
- **录制同理（2026-09-12 §15 踩坑）**：`record.py --case` 局部重录会把该用例
  录在**冷池态**，而全量 verify 跑到它时池已热 → C-h264-cpu 在 19/1108 个
  刀锋段上置信度末位翻转、`ocr.segments` DIFF（结构/文本恒同）。**重录一律
  整矩阵同序**（`record.py` 不带 `--case`），录制与验证的池温轨迹才一致。
