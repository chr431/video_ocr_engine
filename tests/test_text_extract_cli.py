"""text_extract_cli 单元测试（无需视频/decord/GPU）：CSV 行构建、时间戳、写文件、参数解析。"""
from __future__ import annotations

import csv
from pathlib import Path

import text_extract_cli as m
...
def test_build_rows_maps_timestamp_and_skips_empty(monkeypatch):
    monkeypatch.setenv("RVTOL_SEG_GAMMA", "0")  # 无关，仅防环境干扰
...
def test_default_output_path():
    assert m.default_output_path(Path("D:/x/abc.mp4")) == Path("D:/x/abc_ocr.csv")
