...
test6 同内容三编码族**各 23970 帧全片**（≈400s@60fps）、`ocr_backend="tensorrt"`、
`tools/bench.py ab` 交错配对 4/6 轮热轮中位（时钟门禁豁免的 cpu 臂
`--no-clock-gate`），全部符号一致（4/4 或 6/0）。换机需按
`docs/CONCLUSIONS.md` C-04/C-05/C-08 的复评条件重测）：

| 片源编码 | `cpu` | `nvdec` | `hybrid` | `auto`（=NVDEC） | 建议 |
|---|---:|---:|---:|---:|---|
| **h264**（test6_h264） | 11.11 s | 25.86 s | **8.55 s** | 25.86 s | **`hybrid`**（比最优单臂 cpu 快 1.30×） |
| **hevc**（test6_hevc） | 27.55 s | 11.89 s | **9.03 s** | 11.89 s | **`hybrid`**（比 nvdec 快 1.32×） |
| **av1**（test6.mp4） | 21.65 s | 14.23 s | **10.07 s** | 14.23 s | **`hybrid`**（比 nvdec 快 1.41×） |
...
> 各自最优单臂的片长）：**h264（test6 族）全片反超 22%；hevc 交叉点
