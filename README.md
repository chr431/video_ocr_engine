...
> **视频长度依赖**（2026-09-13 实测：hybrid vs 纯 NVDEC，同刻两臂、各自独立
> 子进程、min-of-2；证据与口径见 `docs/log/2026-09-13-hybrid解码率与理论并联和.md`）：
> **h264 无交叉点**——500 帧起 hybrid 即胜，6000 帧领先 +111%；
> **hevc / av1 有交叉点，位于 1500~3000 帧**（≈13~25 秒 @120fps）——更短的片段上
> 纯 NVDEC 反而快约 **0.17s**（该差额已定位为"慢腿首个采样块在保序交付上的暴露"），
> 更长则 hybrid 稳定领先（6000 帧 +12% / +40%）。故上表 hevc 的"持平"含窗口效应。
> 对纯 CPU 臂，hybrid 在三编码的全片口径上均已胜出（1.10× / 2.76× / 1.81×）。
